-- 부서명이 지원팀인 직원의 사원번호, 사원명, 부서코드, 부서명 출력

/*
 * (최소) 조인조건 개수 = 조인되는 테이블 수 - 1
 * 			1	   =		2 - 1
 * 			2      =		3 - 1
 * 			3	   = 		4 - 1
 */
...
-- INNER JOIN
-- 주소가 수원인 직원의 사원번호, 사원명, 주소, 부서코드, 부서명을 출력
SELECT A.EMP_NO 
	,A.EMP_NAME 
	,A.ADDR 
	,A.DEPT_CD 
...
-- 부서명이 지원팀인 직원의 사원번호, 사원명, 부서코드, 부서명, 보유자격증을 출력
SELECT A.EMP_NO 
	,A.EMP_NAME 
	,A.DEPT_CD 
	,B.DEPT_NM 
...
SELECT A.EMP_NO 
	...
	,C.CERTI_CD
FROM TB_EMP A JOIN TB_DEPT B ON (A.DEPT_CD = B.DEPT_CD)
 			  JOIN TB_EMP_CERTI C ON (A.DEPT_CD = B.DEPT_CD)
AND A.EMP_NO = C.EMP_NO
AND B.DEPT_NM = '지원팀'
;
...
DELETE 
	FROM TB_DEPT
	WHERE DEPT_CD IN ('100014', '1000015');
...
-- RIGHT OUTER 조인
-- 현재 아무런 사원을 갖고 있지 않은 부서도 모두 출력
-- TB_DEPT(RIGHT)는 모두 나오고 TB_EMP는 매칭되는 집합만 출력
SELECT A.EMP_NO
	  ,A.EMP_NAME 
	  ,B.DEPT_CD 
	  ,B.DEPT_NM 
FROM TB_EMP A
	,TB_DEPT B
WHERE B.DEPT_CD IN ('100014', '1000015','100001')
 AND A.DEPT_CD(+) = B.DEPT_CD
 ;


--ANSI 조인 방식 (RIGHT OUTER 조인)
SELECT A.EMP_NO
	  ,A.EMP_NAME 
	  ,B.DEPT_CD 
	  ,B.DEPT_NM 
...
INSERT INTO EZEN.SINGER
VALUES('오리', 2);


UPDATE SINGER SET NUM = WHERE NAME = '';


DELETE FROM EZEN.SINGER
WHERE NAME ='오리';


SELECT * FROM SINGER WHERE NUM = 4;
